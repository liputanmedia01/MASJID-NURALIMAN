...
interface StructureManagerProps {
    items: StructureItem[];
    setItems: React.Dispatch<React.SetStateAction<StructureItem[]>>;
}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {items.map(item => (
                        <div key={item.id} className="border p-4 rounded-lg">
                            ...
                            <button onClick={() => setEditingItem(item)} className="text-sm text-blue-600 font-semibold mt-4">Edit</button>
                        </div>
                    ))}
                </div>
            </div>
